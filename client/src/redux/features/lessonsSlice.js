...
import {
  sortAtoZ,
  sortZtoA,
  sortEasiestToHardest,
  sortHardestToEasiest,
...
const fetchAllLessons = createAsyncThunk(
  ...
  async () => {
    try {
      const response = await axios.get(`${URL}/lessons`, config);
      return response.data;
    } catch (error) {
      console.log("llegamos aca cuando entra un usuario sin permisos");
  ...
  reducers: {
    ...
    sortByType: (state, action) => {
      ...
      if (filteredLessons.length === 0)
        state.error = `No se encontraron clases con los tipos: ${typesArray.join(
          ", "
        )}.`;
    ...
    sortByIntensity: (state, action) => {
      ...
      state.lessons = filteredLessons;
      if (filteredLessons.length === 0)
        state.error = `No se encontraron clases con los filtros seleccionados: ${intensity.join(
          ", "
        )}`;
      if (filteredLessons.length > 0) state.error = "";
    },
    ...
    sortByIntensityandType: (state, action) => {
      ...
      const filteredLessons = lessons.filter((lesson) => {
        const lessonTypes = lesson.exercisesTypes;
        return (
          selectedTypes.every((type) => lessonTypes.includes(type)) &&
          selectedIntensities.some((int) => int === lesson.effort)
        );
      });

      state.lessons = filteredLessons;
      if (filteredLessons.length === 0)
        state.error = `No se encontraron clases de tipo de ejercicio ${selectedTypes.join(
          ", "
        )} y de intensidad ${selectedIntensities.join(", ")}`;
      if (filteredLessons.length > 0) state.error = "";
    },
    clearLessons: (state) => {
      state.lessons = [];
...
export const {
  orderFromAtoZ,
  orderFromZtoA,
  orderFomHardestToEasiest,
  orderFromEasiestToHardest,
  sortByType,
  sortByIntensityandType,
  sortByIntensity,
  clearLessons,
